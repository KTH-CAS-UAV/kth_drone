...
int main(int argc, char **argv) {
    ...
    for (YAML::const_iterator view=config["views"].begin();view!=config["views"].end();++view) {
        ...
        //origin = Eigen::Translation3d(-2.,-2.,2.)*Eigen::Scaling(1.,0.,1.);
        //origin = Eigen::Translation3d(-2.,-2.,1.)*
          //      cloud->sensor_orientation_.cast<double>();
        std::cout << "**********ori: " << origin.translation() << "rotation: " << origin.rotation() << std::endl;
        //ROS_ERROR_STREAM("This is my index: " << cloud->sensor_orientation_);
